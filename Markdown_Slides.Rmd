...
```{r}
colnames(var1)
colnames(var1) <- c("mpg", "cyl",  "disp", "hp", "drat", "wt", "qsec", "vs", "am", "gear1", "carb")
setnames(var1, "gear1", "gear")
```


* Removing one data set or all data sets from 'Environment'
```{r}
rm(var1)
rm(list = ls())
```
...
data1 <- read.table("Output/data1.txt", header = TRUE, sep = "|")
data2 <- read.csv(file = "Output/data.csv")
download.file("https://www.nseindia.com/content/historical/EQUITIES/2018/JUL/cm30JUL2018bhav.csv.zip", "NSE BhavCopy/cm30JUL2018bhav.csv.zip")
```
